...
bool GroundStation::readReceivedPacket()
{
    ...
    // We also need to verify that the FC RSSI and SNRs are there
    if (!currentFrameView.hasAtomic(AT_FLIGHT_ATOMIC))
    {
        LOGGING(SPED, "No atomic fc packet detected");
        return false;
    }
    ...
    if (p && currentFrameView.hasAtomic(AT_FLIGHT_ATOMIC))
    {
        ...
        FC_SNR = flight.fc_snr * 0.25f;
        FC_LastTime = flight.gps_time_last_update;
        return true;
    }
    return false;
}
